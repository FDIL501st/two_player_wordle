...
/// Combines two `LetterState` values within a guess, returning the state that will be kept in the backend.
...
/// Combines two encoded `LetterState` values within a guess, returning the state that will be kept in the backend.
/// The order of states stored is: GREEN > YELLOW > BLACK.
...
#[cfg(test)]
mod combine_letter_states_tests {
    use super::*;
    ...
    fn test_combine_black_black_output_black() {
        assert_that!(combine_guess_letter_states(
            LetterState::BLACK,
            LetterState::BLACK
        ))
        .is_equal_to(LetterState::BLACK);
    }
    #[test]
    fn test_combine_yellow_yellow_output_yellow() {
        ...
        .is_equal_to(LetterState::YELLOW);
    }
    #[test]
...
mod prepare_guess_tests {
    ...
    fn prepare_guess_no_duplicate_letters_same_output_state() {
        ...
        let mut actual_pairs: HashSet<LetterStatePair> =
            HashSet::with_capacity(expected_pairs.len());
...
mod encode_guess_tests {
    ...
    fn encode_guess_only_update_guess_letters_others_mixed_states() {
        ...
        let mut actual_u54: U54 = U54::from(
